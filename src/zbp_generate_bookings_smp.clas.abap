CLASS zbp_generate_bookings_smp DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zbp_generate_bookings_smp IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF ztbooking_smp.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ztbooking_smp.
*   insert the new table entries
    INSERT ztbooking_smp FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ztbooking_smp INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
